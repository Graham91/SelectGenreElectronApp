...
- **Features**:
  ...
  - **NEW: Instrumental Track Handling**: Automatically marks tracks as "[Instrumental]" when no lyrics are found, preventing repeated scraping attempts
  - **Smart Skip Logic**: Recognizes "[Instrumental]" as valid lyrics content to avoid re-processing
...
- **Intelligent Lyrics Handling**: 
  - **Found Lyrics**: Updates MP3 files with complete lyric text
  - **No Lyrics Found**: Automatically sets lyrics to "[Instrumental]" to mark the track as processed
  - **Prevents Re-scraping**: Files marked "[Instrumental]" are recognized as complete and skipped in future runs
- **Metadata Integration**: Updates MP3 files with scraped lyrics and genres using NodeID3
- **Smart Processing**: Skips files that already have complete lyrics and genres (including "[Instrumental]")
...
#### **Instrumental Track Logic**
The scraper now intelligently handles tracks without lyrics:

**Problem Solved**: Previously, tracks without lyrics would be scraped repeatedly because empty lyrics fields made them appear "incomplete."

**New Behavior**:
1. **Successful Scrape with No Lyrics**: If genre information is found but no lyrics are detected, the system automatically sets the lyrics field to "[Instrumental]"
2. **Skip Recognition**: The system recognizes "[Instrumental]" as valid lyrics content, preventing future scraping attempts
3. **Logging**: Clear log messages indicate when files are marked as instrumental: `"No lyrics found, marked as instrumental"`
4. **Efficiency**: Eliminates infinite re-scraping of the same instrumental tracks

**Example Flow**:
```
File: "Song.mp3" (empty lyrics, has Suno URL)
First Scrape: Finds genres but no lyrics → Sets lyrics to "[Instrumental]"  
Future Scrapes: Detects "[Instrumental]" → Skips file (already complete)
Result: Instrumental tracks are processed once and never re-scraped
```
...
#### **Smart Numbering & Library Management**
**NEW: Intelligent Number Preservation** - The system now intelligently handles library updates by preserving existing song numbers and only assigning new numbers to genuinely new files.

**Key Features:**
- **Album-Based Grouping**: Numbers are assigned and preserved within each album, preventing duplicate track numbers  
- **Existing Number Detection**: Scans directory for files already processed by each rule, grouped by album
- **Number Preservation**: Files that were previously renamed by a rule keep their original numbers within their album
- **Smart New Assignment**: New files get the next available numbers in their respective albums
- **No Re-numbering**: Eliminates the problem of existing songs getting new numbers during library updates
- **Conflict-Free Updates**: You can add new songs without affecting existing organization
- **Dynamic Name Support**: Handles song names with genres or other dynamic elements correctly

**How It Works:**
1. **Album Recognition**: For each rule, the system reads existing file metadata to identify album groupings
2. **Pattern Recognition**: Files are grouped by the album name that would be generated by the rule's album template
3. **Directory Scan**: Existing files are analyzed within each album group to identify numbering patterns
4. **Number Extraction**: Numbers are extracted from existing filenames and tracked per album
5. **Preservation Logic**: Files already matching the pattern keep their assigned numbers within their album
6. **Sequential Assignment**: New files get assigned the next available numbers within their target album

**Example Scenario:**
```
Existing Library:
Album: "Rock Collection"
- 001 - Artist - Song A [Rock].mp3 (from Rule 1)
- 002 - Artist - Song B [Jazz,Rock].mp3 (from Rule 1) 
- 003 - Artist - Song C [Metal,Rock].mp3 (from Rule 1)

After Adding New Songs to Same Album:
Album: "Rock Collection"  
- 001 - Artist - Song A [Rock].mp3 (number preserved)
- 002 - Artist - Song B [Jazz,Rock].mp3 (number preserved)
- 003 - Artist - Song C [Metal,Rock].mp3 (number preserved)
- 004 - Artist - New Song D [Blues,Rock].mp3 (new number assigned)
- 005 - Artist - New Song E [Folk,Rock].mp3 (new number assigned)

Different Album Gets Its Own Numbering:
Album: "Jazz Collection"
- 001 - Artist - Jazz Song A [Jazz].mp3 (starts from 1 for different album)
- 002 - Artist - Jazz Song B [Smooth Jazz].mp3
```
